        ...
        <div class="order-panel-content">
            ...
            <div class="row page-tilte align-items-center order-panel-header">
                <div class="col-md-auto">
                    ...
                    <h1 class="weight-300 h3 title">Set Customer </h1>
                </div> 
                <div class="col controls-wrapper mt-3 mt-md-0 d-none d-md-block ">
                    <div class="controls d-flex justify-content-center justify-content-md-end">
                        <input type="search" class="form-control d-inline-block no-shadowed" placeholder="Search customer...">
                        <a v-if="selectedCustomer!==null" @click="setCustomer()" class="btn mb-2 circle btn-danger cl-white" title="Set Customer" ><span class="material-icons align-text-bottom cl-white">perm_identity</span> Set Customer</a>
                    ...
                    <div class="col-lg-4 mb-4">
                        <div class="card h-100">
                            <div v-if="selectedCustomer" class="card-body">
                                ...
                                <button @click="setCustomer()" class="btn btn-danger px-4 circle mx-1 no-shadowed">Set Customer</button>
                                </div>
                            ...
                            <div v-else class="card-body">
                                ...
                                <div class="text-center">
                                    <button data-toggle="modal" data-target="#newCustomerModal" :class="{'btn px-4 circle mx-1 cl-white btn-info no-shadowed':true}">Add Customer</button>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card">
                            <table class="table responsive-table table-bordered bg-white">
                                <thead class="thead-light2">
                                    <tr>
                                        <th scope="col" class="resizeable">Full Name</th>
                                        <th scope="col" class="resizeable">Address</th>
                                        <th scope="col" class="resizeable">Mobile</th>
                                    </tr>
                                </thead>
                                <tr v-for="(customer,index) in customers" @click="selectCustomer(customer)" :key="'customer_'+index" :class="{'pointer-cursor':true,'active':selectedCustomer!==null&&selectedCustomer.id===customer.id}">
                                    <td data-label="Order#">{{customer.first_name+' '+customer.last_name}}</td>
                                    <td data-label="Reference">{{customer.address}}</td>
                                    <td data-label="Date">{{customer.mobile}}</td>
                                </tr>
                            </table>
                        </div>
                    </div>
            ...
            <add-customer :modalId="'newCustomerModal'" :newCustomerEmitter="newCustomerEmitter" @customer-created-event="getCustomer(newCustomerEmitter)"/>
        </div>
    ...
    methods: {
        clientBackBtn(){
            if(this.calledfrom==='POS'){
                ...
                this.$emit('customer-payment-cancel-event');
            }
        },
