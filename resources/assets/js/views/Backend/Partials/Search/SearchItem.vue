<template>
    <div v-bind:class="{'padding-bottom-10 padding-right-10 padding-left-10 form-group':barcode,'bg-eee':bg_eee}" v-bind:style="{ width: field_cover_wdth + 'px'}">
        <label v-if="show_label" class="fs-12 mgb-0"><i v-if="!searching_list" class="fa fa-barcode" aria-hidden="true"></i><i v-if="searching_list" class="fa fa-spinner fa-spin"></i>  SCAN BARCODE OR SEARCH ITEMS</label>
        <input type="text" :disabled="loading_list" v-model="query" class="form-control br-2 no-shadowed  height-30" v-bind:style="{ width: field_wdth + '%'}" autofocus="autofocus"/>
        <div v-if="tabulated_view" id="search_id_result_manual">
            <div v-if="searched_products_list.length">
                <a @click="close_search_result('ADD')" class="cl-blue-link fs-13 fw-600 pointer-cursor">Add</a> | 
                <a @click="close_search_result('CLEAR')" class="cl-amref fs-13 pointer-cursor fw-600 pointer-cursor">Clear</a> | 
                <a @click="close_search_result('CLOSE')" class="cl-amref fs-13 pointer-cursor fw-600 pointer-cursor">Close</a> | 
            </div>
            <ul v-if="queryResults.length" class="search_result" v-bind:style="{ width: search_result_width + 'px',maxHeight:search_result_min_height+'px'}">
                <table class="search-table table-hover">
                    <tr>
                        ...
                        <th style="width:6%" class="fw-600 fs-12 pointer-cursor cl-white text-center">
                            <a @click="close_search_result('CLOSE')"><i class="fa fa-trash-o"></i></a>
                        </th>
                    </tr>
                    <tr v-for="(result,index) in queryResults" :key="'queryResults'+index">
                        ...
                        <td class="uppercase">{{result.brand_sector.name}} - {{+result.single_unit_weight+''+result.measure_unit.slug}}</td>
                        <td class="text-right padding-right-5">{{formatMoney( (result.stock_packs*result.units_per_pack) + result.stock_units )}}</td>
                        <td class="text-right padding-right-5">{{formatMoney(result.price.unit_retail_price)}}</td>
                        <td class=""></td>
                    </tr>
                </table>
            </ul>
        </div>
        <div v-else id="search_id_result_manuals">
...
<script>
    ...
    import {SEARCH_ITEM_URL,PRODUCT_ITEMS_URL} from '../../../../router/api';
    ...
    export default {
        ...
        methods: {
            ...
            check_all(event){
                if(event.target.checked){
                    ...
                    for (let index = 0; index < this.queryResults.length; index++) {
                        ...
                        for (let w = 0; w < this.product_items.length; w++) {
                            const elementinner = this.product_items[w];
            ...
            loadProductItem(state_show){
                ...
                get(PRODUCT_ITEMS_URL+'?filters=All')
                .then((res) => {
                    if(res.data.status_code === 200) {
                        this.options = res.data.results.data;
...
<style lang="scss" scoped>
    ...
    table.search-table tr td{
     ...
     table.search-table tr td.uppercase{
